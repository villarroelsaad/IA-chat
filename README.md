...
- `back/` — Python (Flask) backend
  - `server.py` — HTTP API with `/health`, `/chat` and `/upload-file` endpoints
  - `chat/chat.py` — Bot implementation: integration with `google.genai` and `SimpleFallbackBot`
  - `test_api.py` — Script to test the `/chat` endpoint
  - `requirements.txt` — Python dependencies
  - `Dockerfile`, `docker-compose.yml` — Files to containerize the project

- `front/` — React + TypeScript (Vite) frontend
  - `src/` — React app code
  - `services/chat.ts` — Requests to the backend (`/chat` and `/upload-file`)
...
- `GEMINI_API_KEY` — (optional) Google GenAI key if you want to use the Gemini service.
- `GEMINI_MODEL` — (optional) default model to use (example: `gemini-2.5-flash`).
- `PORT` — Port for the backend (default: `8000`).
- `VITE_BACKEND_URL` — (frontend) base URI for the backend (example: `http://localhost:8000`).
...
## Installation and running (without Docker) ⚙️
...
## Running with Docker (recommended for quick testing) 🐳
...
## Security notes and limitations ⚠️

- Uploaded file text is stored in memory (`FILE_TEXTS`) — in production you should use persistent storage or a database.
...
- How do I test without installing the Gemini API? — You don't need the key: the backend will fall back to `SimpleFallbackBot`, letting you test uploads and general behavior locally.
